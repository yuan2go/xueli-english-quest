...
export function Scene({ board, selected, select, send, receipt, reduced, assessmentWord }: {
 board: QuestBoard; selected: string | undefined; select: (id: string) => void; send: (intent: Intent) => void; receipt?: Verdict; reduced: boolean; assessmentWord?: string;
}) {
 ...
 useLayoutEffect(()=>{
  ...
  if(!reduced){
   for(const {el,id,x,y} of sampled){
    ...
    let motion=receipt?.motions.find(m=>m.id===id), ancestor=w.entities[id];
    while(!motion&&ancestor.place.kind!=='node') { ancestor=w.entities[ancestor.place.id];motion=receipt?.motions.find(m=>m.id===ancestor.id); }
 ...
 return <div className={`quest-scene level-${board.level}`} ref={scope} aria-label="可操作的纸上世界" data-level={board.level} data-world-revision={w.revision}>
  ...
  {board.level==='R2'&&<div className='paper-arch' aria-hidden='true'/>}
  {board.level==='R1-R'&&<div className='blocked-hole' aria-hidden='true'>纸叶堵住了洞口</div>}
  ...
  {objects.filter(e=>visible(w,e.id)).map(e=>{const p=position(e.id);return <button key={e.id} data-entity={e.id} data-size={e.size} data-place={`${e.place.kind}:${e.place.id}`} data-open={String(e.open)} data-drop={spec.rules.types[e.word].support?`on:${e.id}`:spec.rules.types[e.word].container?`in:${e.id}`:undefined}
   ...
   onPointerDown={event=>pointer.start(event,e.id)} onClick={()=>pointer.click(()=>select(e.id))} aria-label={`${lexeme(e.word)?.zh??e.word}${assessmentWord===e.word?'':` ${e.word}`}`} aria-pressed={selected===e.id}>
   <span className="entity-picture"><Visual id={e.word==='cat'?pose:e.word} label={lexeme(e.word)?.zh??e.word}/></span>
   <span className="entity-label">{lexeme(e.word)?.zh} <b lang="en">{assessmentWord===e.word?'':e.word}</b></span>
