...
const value = (table: [number, number, number] | undefined, size: Size) =>
  table?.[extent(size) - 1] ?? 0;
export function thing(w: SpatialWorld, id: string): Thing {
  return Object.hasOwn(w.entities, id)
    ? w.entities[id]
    : fail("REFERENCE", "这个物品已经不在这里了。");
}
...
export function children(w: SpatialWorld, id: string): Thing[] {
  return Object.values(w.entities).filter(
    (e) => e.place.kind !== "node" && e.place.id === id,
  );
}
  ...
  while (queue.length) {
    ...
    for (const edge of rules.edges) {
      ...
      if (
        ...
        (edge.door && !thing(w, edge.door).open) ||
        clearance > rules.nodes.find((n) => n.id === next)!.clearance
      )
        continue;
      visited.add(next);
...
function requirePath(
  ...
  return (
    path(w, r, from, to, clearance) ??
    fail("PATH", "这里过不去：看看门、洞口，以及小猫和物品的大小。")
  );
}
function type(r: SpatialRules, e: Thing) {
  return Object.hasOwn(r.types, e.word)
    ? r.types[e.word]
    : fail("TYPE", "这里还没有这种词语工具。");
}
function relation(w: SpatialWorld, r: SpatialRules, e: Thing) {
  if (e.place.kind === "node") {
    const n = r.nodes.find((n) => n.id === e.place.id);
    if (!n || extent(e.size) > n.clearance)
      fail("CLEARANCE", "这里太窄，先换个地方再改变大小。");
    return;
  }
  nodeOf(w, e.id);
  const p = thing(w, e.place.id),
    a = type(r, p);
  if (e.place.kind === "in") {
    if (!a.container || type(r, e).actor)
      fail("CONTAINER", "这个对象不能这样装进去。");
    const load = children(w, p.id)
      .filter((c) => c.place.kind === "in")
      .reduce((n, c) => n + extent(c.size), 0);
    if (extent(e.size) > extent(p.size) || load > value(a.capacity, p.size))
      fail("CAPACITY", "装不下了：试试改变物品或容器的大小。");
  } else {
    if (!a.support || extent(e.size) > value(a.bearing, p.size))
      fail("SUPPORT", "这个支撑太小，托不住。先调整大小。");
  }
}
export function assertSpatial(w: SpatialWorld, r: SpatialRules) {
  if (
    ...
    fail("WORLD", "世界版本无效。");
  if (
    new Set(w.created).size !== w.created.length ||
    w.created.some((id) => !r.quotas.some((q) => q.id === id))
  )
    fail("QUOTA", "制作记录无效。");
  let actors = 0;
  for (const [id, e] of Object.entries(w.entities)) {
    if (
      ...
      !["small", "normal", "big"].includes(e.size) ||
      typeof e.open !== "boolean"
    )
      fail("WORLD", "物品记录无效。");
    if (type(r, e).actor) {
      actors++;
      if (id !== "cat-companion") fail("IDENTITY", "伙伴只有这一只。");
    }
    relation(w, r, e);
  }
...
export function applySpatial(
  ...
  if (action.type === "create") {
    ...
    if (!quota || type(r, { word: action.word } as Thing).actor)
      fail("QUOTA", "这里不能制作这个物品。");
    ...
    next.entities[quota.id] = {
      id: quota.id,
      word: quota.word,
      size: "normal",
      open: true,
      place: { kind: "node", id: quota.node },
    };
  } else {
    const e = thing(next, action.source),
      a = type(r, e);
    if (!visible(next, e.id))
      fail("CLOSED", "物品还在关闭的容器里。先打开它。");
    if (action.type === "resize") {
      if (!a.resize) fail("FIXED", "这个物品固定在场景中，不能改变大小。");
      if (children(next, e.id).some((c) => c.place.kind === "on"))
        fail("OCCUPIED", "上面还有东西。先下来或搬开，再改变大小。");
      e.size = action.size;
    } else if (action.type === "open") {
      const handles = r.handles.filter((h) => h.door === e.id);
      if (!a.container && !handles.length)
        fail("OPEN", "这个物品没有可以打开的部分。");
      const from = nodeOf(next, cat.id);
      if (handles.length) {
        const candidates = handles.map((h) => {
          const route = path(next, r, from, h.node, extent(cat.size));
          const support =
            cat.place.kind === "on" && from === h.node
              ? thing(next, cat.place.id)
              : undefined;
          const height =
            extent(cat.size) +
            (support ? value(type(r, support).height, support.size) : 0);
          return { h, route, height };
        });
        const allowed = candidates.find(
          (c) => c.route && c.height >= c.h.height,
        );
        if (!allowed)
          fail("HANDLE", "小猫够不到把手。高处需要支撑，另一侧也许有办法。");
        if (from !== allowed.h.node)
          cat.place = { kind: "node", id: allowed.h.node };
        motions.push({ id: cat.id, nodes: allowed.route! });
      } else {
        const target = nodeOf(next, e.id),
          route = requirePath(next, r, from, target, extent(cat.size));
        cat.place = { kind: "node", id: target };
        motions.push({ id: cat.id, nodes: route });
      }
      e.open = action.open;
    } else if (action.type === "move") {
      if (a.fixed) fail("FIXED", "它固定在这里，小猫可以走过去操作。");
      const target =
        action.to.kind === "node" ? action.to.id : nodeOf(next, action.to.id);
      if (!r.nodes.some((n) => n.id === target))
        fail("REFERENCE", "这里没有落点。");
      if (action.to.kind !== "node") {
        if (action.to.id === e.id) fail("CYCLE", "物品不能放进自己。");
        const parent = thing(next, action.to.id);
        if (
          !visible(next, parent.id) ||
          (action.to.kind === "in" && !parent.open)
        )
          fail("CLOSED", "先打开容器，再放进去。");
      }
      const from = nodeOf(next, cat.id),
        source = nodeOf(next, e.id);
      const approach = requirePath(next, r, from, source, extent(cat.size));
      if (!a.actor && extent(e.size) > extent(cat.size) + 1)
        fail("CARRY", "现在的小猫搬不动这么大的物品。");
      if (children(next, e.id).some((c) => c.place.kind === "on"))
        fail("OCCUPIED", "上面还有东西，先移开再搬运。");
      const route = requirePath(
        next,
        r,
        source,
        target,
        Math.max(extent(cat.size), extent(e.size)),
      );
