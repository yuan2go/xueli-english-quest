...
首个故事《小猫的野餐冒险》是一款 Scene-first 触屏绘本游戏。正式入口 `/` 已接入场景目标驱动的三幕：家门口准备 → 湿墨小径 → 野餐草地。没有账号、后端或运行时 AI 依赖。
...
世界是持续主界面：点物品或场景邀请才打开拼写、组句、听音与行动工具；关闭即回到世界，不自动连着做下一题。手机工具内部滚动，场景持续可见；词块可以拖入、排序和退回。变形、移动、过路和抵达在业务提交后呈现，刷新或跳过不影响结果。

探索小路中的三个短活动按能力提前开放，各有两种手工编排情境：帽子搭配、背包找物、野餐小帮手。退出回到原故事布置，刷新不改活动变体；重玩切换另一种情境。自由制作的备用 mat / hat 各一件，稳定 ID，不复制主角或关键物品。
