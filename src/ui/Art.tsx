...
export function Visual({
  ...
  if (broken || fail || assets.failed.includes(id))
    return (
      <span
        className={`art-fallback ${className}`}
        role="img"
        aria-label={`${label}，图像暂不可用`}
      >
        <span>{label}</span>
        <small>图像暂不可用 · 可重试</small>
      </span>
    );
  return (
    <img
...
export function CharacterArt({
  ...
  return (
    <span className="character-sprite" data-pose={pose}>
      ...
      {walking && (
        <span className="walk-window" aria-hidden="true">
          <Visual id="cat-walk" label="狸花猫走路" className="walk-sheet" />
        </span>
      )}
      {previous && (
        <span key={visible} className="character-previous" aria-hidden="true">
...
export function AssetNotice() {
  ...
  return (
    <div className="notice resource-notice" role="alert">
      部分插画加载失败，已提供文字替代。进度保留，可以继续。
      <button
        className="quiet"
        disabled={assets.retrying}
