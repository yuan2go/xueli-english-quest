...
export function puzzle(id: string, variant = 0): Puzzle {
  if (id === 'R2') return {
    id, title: '装得下，也带得走', subtitle: '纸上小径 · 第二页',
    invitation: '苹果和小背包不太合适。调整它们，或做个箱子，把水果一起带到野餐路口。',
    goalText: '把苹果装进背包或箱子，带到野餐路口',
    rules: { types: TYPES, nodes: [
      ...
      {a:'home',b:'packing',clearance:3,enabled:true}, {a:'packing',b:'tunnel',clearance:2,enabled:true}, {a:'tunnel',b:'clearing',clearance:2,enabled:true},
    ], handles: [], quotas: [{id:'craft-box',word:'box',node:'home'}] },
    initial: initial([entity('cat-companion','cat','home'),entity('bag-main','bag','home','small',false),entity('apple-main','apple','packing')]),
    goals: [{type:'delivered',id:'apple-main',node:'clearing',containers:['bag-main','craft-box']},{type:'at',id:'cat-companion',node:'clearing'}],
    words:['apple','bag','box','open','close','in','on','put','is','small','big'],
  ...
  if (id === 'R3') {
    ...
    rules.handles=[{door:'gate',node:'step',height:2},{door:'gate',node:'inside',height:1}];
    rules.edges.filter(e=>e.a==='hole'||e.b==='hole').forEach(e=>{e.enabled=false;});
    rules.nodes=rules.nodes.filter(n=>n.id!=='hole');rules.edges=rules.edges.filter(e=>e.a!=='hole'&&e.b!=='hole');
    rules.quotas=[];
    return {
      id,title:'朋友的一句请求',subtitle:'纸上小径 · 第三页',invitation:'朋友说了一句英语。先听听它要什么，再决定放在哪里。',goalText:'按朋友的请求送好苹果，让小猫来到花园',rules,
      initial:initial([entity('cat-companion','cat','home'),entity('apple-main','apple','home'),entity('bag-main','bag','step'),entity('gate','door','step','normal',false),entity('basket-main','basket','garden'),entity('box-main','box','inside'),entity('picnic-mat','mat','garden','big')]),
      goals:[{type:'relation',id:'apple-main',kind:relation,target},{type:'at',id:'cat-companion',node:'garden'}],
      words:['apple','basket','box','mat','door','open','in','on','put','is'],hint:'听清 in 还是 on；一句是在里面，另一句是在上面。请先打开路。',
      request:relation==='in'?'Put the apple in the basket.':'Put the apple on the box.',requestRelation:relation,
    };
  }
  if (id === 'workshop') return {
    id,title:'词语的魔法工坊',subtitle:'纸上小径 · 随时来试试',invitation:'这里的物品可以反复试。带走的是你的发现，物品留在工坊。',goalText:'认识意义，然后用自己的句子试一试',
    rules:{types:TYPES,nodes:[{id:'home',label:'制作台',x:18,y:70,clearance:3},{id:'bench',label:'试验台',x:51,y:65,clearance:3},{id:'display',label:'展示处',x:82,y:72,clearance:3}],edges:[{a:'home',b:'bench',clearance:3,enabled:true},{a:'bench',b:'display',clearance:3,enabled:true}],handles:[],quotas:[{id:'craft-box',word:'box',node:'home'},{id:'craft-apple',word:'apple',node:'display'}]},
    initial:initial([entity('cat-companion','cat','home'),entity('apple-main','apple','home'),entity('box-main','box','bench'),entity('bag-main','bag','display'),entity('picnic-mat','mat','display','big')]),
    goals:[],words:LEXICON.map(l=>l.word),hint:'试试把苹果放进箱子，再用 The … is … 描述它。',
  };
  ...
  return {
    id, title: id === 'R1-R' ? '再访 · 雨后的小花园' : '围栏那边的篮子', subtitle: id==='R1-R'?'纸上小径 · 雨后回访':'纸上小径 · 第一页',
...
export const EXERCISES = {
 command:{id:'apple-in-box-command',meaning:{kind:'command',verb:'place',source:'apple',relation:'in',target:'box'},prompt:'请用一句指令，把苹果放进箱子。',example:'Put the apple in the box.',requires:['apple','box','put','in'],dimension:'grammar'},
 description:{id:'apple-in-box-description',meaning:{kind:'description',verb:'place',source:'apple',relation:'in',target:'box'},prompt:'先自己布置苹果在箱子里，再说出你看见的位置。描述不能移动物品。',example:'The apple is in the box.',requires:['apple','box','is','in'],dimension:'grammar'},
 spelling:{id:'create-box',prompt:'根据意思和发音，拼出「箱子」，做一个可以使用的物品。',example:'box',requires:['box'],dimension:'spelling'},
} as const;
export function relationIs(place: Place, kind: 'in' | 'on', id: string) { return place.kind === kind && place.id === id; }
export const QUEST_AUDIO = [...new Set([...LEXICON.flatMap(l => [l.word, l.example]), 'Put the apple in the basket.', 'Put the apple on the box.', 'The apple is in the box.'])].map((text, i) => ({
