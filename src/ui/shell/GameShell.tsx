...
import { puzzle, LEXICON, lexeme, EXERCISES } from '../../content/quest.ts';
...
export function GameShell({ session, send, receipt, reduced, speak }: {session:Quest;send:(i:Intent)=>void;receipt?:Verdict;reduced:boolean;speak:(text:string)=>void}) {
 ...
 function practice(mode:'assisted'|'independent',type:'command'|'description'|'spelling'){
  send({kind:'practice',mode,exercise:type});setTool(type==='spelling'?'spell':'sentence');setWord('box');
 }
 function demonstrate(){
  send({kind:'support',value:'demo'});
  if(b.level==='workshop'){
   send({kind:'world',action:{type:'open',source:'box-main',open:true}});
   send({kind:'sentence',task:taskId(session),text:EXERCISES.command.example,selected:'box-main'});
  }
 }
 const sampleTarget=Object.values(b.world.entities).find(e=>e.word===word)?.id??(b.world.entities['craft-box']?'craft-box':'cat-companion');
 ...
 return <main className={`game-shell ${tool?'has-tool':''}`}>
  ...
  {spec.request&&<div className="request-slip"><span>朋友的小纸条</span><button onClick={()=>speak(spec.request!)}>听朋友的请求</button><button onClick={()=>send({kind:'support',value:'text'})}>看文字辅助</button>{b.support.includes('text')&&<p lang="en">{spec.request}</p>}<small>开发语音未听审；可用文字继续，按辅助记录。</small></div>}
  <Scene board={b} selected={selected} select={choose} send={send} receipt={receipt} reduced={reduced} assessmentWord={b.practice==='independent'&&tool==='spell'?word:undefined}/>
  {b.level==='workshop'&&<section className="workshop-choices" aria-label="工坊练习"><p>先认识意义，再挑一种方式试用</p><div>{(['command','description','spelling'] as const).map(type=><span key={type}><b>{type==='command'?'句子行动':type==='description'?'观察描述':'拼写造物'}</b><button onClick={()=>practice('assisted',type)}>辅助{type==='command'?'指令':type==='description'?'描述':'拼写'}</button><button onClick={()=>practice('independent',type)}>独立{type==='command'?'指令':type==='description'?'描述':'拼写'}</button></span>)}</div></section>}
  ...
  {done&&b.level!=='workshop'&&<div className="chapter-arrival">{b.level==='R1'||b.level==='R2'?<button className="primary" onClick={()=>{setTool(null);setSelected(undefined);send({kind:'next'});}}>沿小径继续 →</button>:<><h2>这一份心意，送到了。</h2><p>苹果{b.world.entities['apple-main']?.place.kind==='in'?'在篮子里面':'在箱子上面'}，小猫已经来到朋友身边。这是你实际留下的布置。</p><button onClick={()=>setTool('sentence')}>用描述回望布置</button><button onClick={()=>send({kind:'enter',mode:'revisit'})}>雨后，再回花园</button></>}</div>}
  {tool&&<aside className="context-tool" aria-label="英语工具抽屉" onKeyDown={e=>{if(e.key==='Escape')close();}}><header><h2>{tool==='spell'?'WordSpell · 拼词造物':tool==='sentence'?'让英语做点事情':tool==='learn'?'词义小样':selectedEntity?`${lexeme(selectedEntity.word)?.zh} · ${selectedEntity.word}`:'选一个物品'}</h2><button aria-label="关闭工具" onClick={close}>×</button></header><div className="tool-scroll">
   ...
   {tool==='learn'&&<><div className="meaning-tabs">{spec.words.map(w=><button key={w} aria-pressed={word===w} onClick={()=>teach(w)}>{w}</button>)}</div><div className="meaning-sample"><Visual id={lexeme(word)?.asset??'box'} label={lexeme(word)?.zh??word}/><div><h3>{word} · {lexeme(word)?.zh}</h3><p>{lexeme(word)?.meaning}</p><button onClick={()=>speak(word)}>听词语</button></div></div><div className="meaning-comparison"><span><Visual id={lexeme(word)?.asset??'box'} label="小的示意"/><b>small</b></span><span><Visual id={lexeme(word)?.asset??'box'} label="大的示意"/><b>big</b></span></div>
   {['small','big'].includes(word)&&<button onClick={()=>send({kind:'world',action:{type:'resize',source:sampleTarget,size:word as 'small'|'big'}})}>在场景试试 {word}</button>}
   {['open','close'].includes(word)&&<button onClick={()=>send({kind:'world',action:{type:'open',source:b.world.entities['box-main']?'box-main':b.world.entities['bag-main']?'bag-main':'gate',open:word==='open'}})}>在场景试试 {word}</button>}
   {['in','on'].includes(word)&&b.world.entities['apple-main']&&b.world.entities['box-main']&&<button onClick={()=>send({kind:'world',action:{type:'move',source:'apple-main',to:{kind:word as 'in'|'on',id:'box-main'}}})}>在场景试试 {word}</button>}
   <p>Open / Put / Make 请伙伴行动；The … is … 说眼前的情况。</p><button className="primary" onClick={()=>{setTool('object');setSelected('cat-companion');}}>回到场景试一试</button></>}
   {tool==='spell'&&<>{b.level==='workshop'&&b.practice==='assisted'&&<p lang='en'>参考词：{word}</p>}<p>用字母做一个{lexeme(word)?.zh}，放在场景里自由使用。</p><div className="action-row">{spec.rules.quotas.map(q=><button key={q.id} onClick={()=>{setWord(q.word);setAnswer('');}}>{lexeme(q.word)?.zh}</button>)}</div>{!b.taught.includes(word)?<button className="primary" onClick={()=>teach(word)}>先认识这个词</button>:<><button onClick={()=>speak(word)}>听一听</button><label className="spell-input">我的字母<input aria-label="我的字母" autoComplete="off" autoCapitalize="none" value={answer} maxLength={20} onChange={e=>setAnswer(e.target.value)}/></label><div className="letter-bank">{(b.practice==='independent'?[...'abcdefghijklmnopqrstuvwxyz']:[...new Set([...word,'a','e','t'])].sort()).map(c=><button key={c} onClick={()=>setAnswer(s=>s+c)} lang="en">{c}</button>)}<button onClick={()=>setAnswer(s=>s.slice(0,-1))}>取回</button></div><button className="primary" disabled={answer.length<word.length} onClick={()=>send({kind:'spell',word,answer})}>施法造物</button></>}</>}
   {tool==='sentence'&&<>{b.level==='workshop'&&b.practice!=='exploration'&&<div className='exercise-request'><p>{exercise.prompt}</p>{b.practice==='assisted'&&<p lang='en'>{exercise.example}</p>}<button onClick={demonstrate}>看一次示范</button></div>}<p>选择指令或描述。含糊的 it 指向你当前选中的物品。</p><SentenceBuilder task={{id:taskId(session),tokens:tokensFor(taskId(session))}} submit={ids=>send({kind:'sentence',task:taskId(session),ids,selected})}/><details><summary>也可以打字</summary><label>我的句子<input aria-label="输入句子" value={sentence} maxLength={240} onChange={e=>setSentence(e.target.value)}/></label><button onClick={()=>send({kind:'sentence',task:taskId(session),text:sentence,selected})}>提交句子</button></details></>}
  </div></aside>}
 </main>;
}
