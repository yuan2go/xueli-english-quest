...
const [baseURL = "http://127.0.0.1:4186", output, label = "working-tree"] =
    ...
    try {
      ...
      await p.evaluate(() => {
        ...
        window.__observer = new MutationObserver((ms) => {
          window.__sample.styles += ms.filter(
            (m) => m.type === "attributes" && m.attributeName === "style",
          ).length;
        });
        window.__observer.observe(document.querySelector(".quest-scene"), {
          attributes: true,
          subtree: true,
      ...
      for (let i = 0; i < 120; i++)
        await p.mouse.move(
          x + Math.sin((i / 119) * Math.PI * 4) * 65,
          y - 40 - Math.sin((i / 119) * Math.PI * 2) * 30,
        );
      ...
      runs.push({
        run,
        ...delta,
        pointerMoves: sample.moves,
        styleMutations: sample.styles,
...
const report = {
  label,
  environment:
    "Chromium headless, 390x844, CDP CPU 4x; actual normal entry wake+bag then 120 pointer moves and Escape; engineering measurement, not physical-device FPS",
  runs,
};
